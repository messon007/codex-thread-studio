...
export class ResourceExtractorRegistry {
    ...
    register(extractor) {
        ...
        if (!id || typeof extractor.extract !== 'function')
            throw new Error('Resource extractor requires an id and extract()');
        if (this.#extractors.some((candidate) => candidate.id === id))
            throw new Error(`Resource extractor already registered: ${id}`);
    ...
    extract(source, context = {}) {
        ...
        for (const extractor of this.#extractors) {
            if (extractor.supports && !extractor.supports(source, context))
                continue;
            let result = [];
            try {
            ...
            for (const candidate of result) {
                if (candidates.length >= MAX_ITEM_OCCURRENCES)
                    break;
...
export class SessionResourceIndex {
    ...
    occurrenceIdsByResource = new Map();
    occurrenceIdsBySource = new Map();
    orderedResourceIds = [];
    truncated = false;
    constructor() {
    ...
    rebuild(model, context = {}, registry = createDefaultResourceRegistry()) {
        ...
        for (const turn of latestTurn ? [latestTurn] : []) {
            for (const item of turn?.items || []) {
                ...
                for (const candidate of registry.extract(source, context)) {
                    ...
                    if (this.occurrencesById.has(occurrence.id))
                        continue;
    ...
    resources({ query = '', kind = 'all' } = {}) {
        ...
        return this.orderedResourceIds.flatMap((id) => {
            ...
            if (!resource)
                return [];
            if (kind !== 'all' && resourceGroup(resource.kind) !== kind)
                return [];
    ...
    counts() {
        ...
        for (const resource of this.resourcesById.values())
            groups[resourceGroup(resource.kind)] += 1;
...
export function buildSessionResourceIndex(model, context = {}, registry = createDefaultResourceRegistry()) {
...
export function normalizeResourceCandidate(candidate, source = {}, context = {}) {
    ...
    if (/^https?:\/\//iu.test(raw))
        return normalizeWebResource({ ...candidate, raw }, source);
...
export function extractUrlCandidates(value) {
    ...
    for (const match of text.matchAll(URL_PATTERN)) {
        if (rangeContains(excludedRanges, match.index))
            continue;
...
export function extractPathCandidates(value) {
    const text = boundedText(value);
    const excludedRanges = markdownCodeBlockRanges(text);
    const output = [];
    for (const match of text.matchAll(FILE_PATTERN)) {
        if (rangeContains(excludedRanges, match.index) || looksLikeMarkupTag(text, match.index, match[0].length))
            continue;
        const raw = cleanCandidate(match[0]);
        if (!looksLikeFilePath(raw, { requireStrongEvidence: true }) || /^https?:[\\/]/iu.test(raw))
            continue;
        output.push({ raw, kind: pathKind(raw), field: 'text', start: match.index, end: match.index + match[0].length, confidence: 0.78 });
    }
    return output;
}
export function resourceGroup(kind) {
    if (['web', 'issue', 'commit'].includes(kind))
        return 'web';
    if (kind === 'code')
        return 'code';
    return 'file';
...
function resourceSourceForItem(turn, item, context) {
    return {
...
function narrativeItemFingerprint(item = {}) {
    ...
    if (previous?.text === text)
        return `${text.length}:${previous.revision}`;
...
function objectRevision(value) {
    if (!value || typeof value !== 'object')
        return 0;
    if (!resourceObjectIds.has(value))
        resourceObjectIds.set(value, nextResourceObjectId++);
...
function extractInlineCodePaths(value) {
    const text = boundedText(value);
    const excludedRanges = markdownCodeBlockRanges(text);
    const output = [];
    for (const match of text.matchAll(INLINE_CODE_PATTERN)) {
        if (rangeContains(excludedRanges, match.index))
            continue;
        const raw = cleanCandidate(match[1]);
        if (!looksLikeFilePath(raw, { requireStrongEvidence: true }))
            continue;
...
function normalizeWebResource(candidate, source) {
    let url;
    try {
        url = new URL(candidate.raw);
    }
    catch {
        return null;
    }
...
function normalizeFileResource(candidate, source, context) {
    const parsed = splitFileLocation(candidate.raw);
    if (!parsed.path || !looksLikeFilePath(parsed.path))
        return null;
...
function arbitrateCandidates(candidates) {
    ...
    for (const candidate of ordered) {
        ...
        const overlap = start != null && end != null && kept.some((existing) => {
            if (String(existing.field || 'text') !== String(candidate.field || 'text'))
                return false;
...
function isNarrativeSource(source) {
    return ['agentMessage', 'plan', 'userMessage'].includes(source.itemType || '');
}
function markdownCodeBlockRanges(value) {
    ...
    for (const line of text.split(/(?<=\n)/u)) {
        ...
        if (!open) {
            ...
            if (match)
                open = { start: offset, marker: match[1][0], length: match[1].length };
        }
        else {
...
function looksLikeMarkupTag(text, start, length) {
...
function trimUrlPunctuation(value) {
    ...
    for (const [left, right] of pairs) {
        while (text.endsWith(right) && count(text, right) > count(text, left))
            text = text.slice(0, -1);
    }
    return text;
}
function occurrenceExcerpt(text, start, end) {
