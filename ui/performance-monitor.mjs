...
export function createPerformanceMonitor({ limit = DEFAULT_RECORD_LIMIT, now = defaultNow, wallNow = Date.now, } = {}) {
    ...
    function report(options = {}) {
        ...
        if (typeof globalThis.console?.table === 'function')
            globalThis.console.table(result.summary);
        return result;
    }
    function clear() {
...
function boundedDetails(details) {
    ...
    for (const [key, value] of Object.entries(details || {})) {
        if (typeof value === 'string')
            output[key] = value.slice(0, MAX_DETAIL_STRING_LENGTH);
