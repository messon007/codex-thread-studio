...
export function replaceComposerTrigger(value, trigger, replacement) {
    if (!trigger)
        return { value, cursor: String(value || '').length };
...
export function isPreviewableTextFile(file) {
    ...
    if (PREVIEWABLE_TEXT_NAMES.has(name) || PREVIEWABLE_TEXT_NAMES.has(stem))
        return true;
...
export function previewableFileKind(file) {
    if (/\.epub$/iu.test(fuzzyFileLabel(file)))
        return 'epub';
    if (/\.pdf$/iu.test(fuzzyFileLabel(file)))
        return 'pdf';
    if (/\.xlsx$/iu.test(fuzzyFileLabel(file)))
        return 'table';
    if (/\.(csv|tsv)$/iu.test(fuzzyFileLabel(file)))
        return 'table';
...
export function transcriptUpdateKind(method) {
    if (method?.toLowerCase().endsWith('delta'))
        return 'stream';
    if (method === 'item/completed')
        return 'item';
    if (method === 'thread/tokenUsage/updated' || method === 'turn/diff/updated' || method === 'thread/status/changed')
        return 'metadata';
    return 'full';
}
