...
export function continuationDraftTurnState(thread) {
    ...
    const output = (turn.items || [])
        ...
        .join('\n');
    if (!output)
        return { status: 'failed', error: 'The session model returned no continuation draft' };
    try {
        const raw = JSON.parse(stripJsonFence(output));
        const parsed = raw !== null && typeof raw === 'object' ? raw : {};
        const prompt = String(parsed?.prompt || '').trim();
        if (!prompt)
            return { status: 'failed', error: 'The session model returned an empty continuation draft' };
        if ([...prompt].length > 4_000)
            return { status: 'failed', error: 'The session model returned a continuation draft longer than 4,000 characters' };
