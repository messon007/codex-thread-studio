import type { SubmissionState } from './submission-controller.mjs'
import type { HiddenUtilityState, UtilityRpcResult } from './hidden-utility-session.mjs'
import type { SessionRef } from './backend-types.mjs'
...
export interface ComposerActionsState extends SubmissionState, HiddenUtilityState {
  ready: boolean; socketGeneration: number; continueBehavior: string; queueDepth: number
  continuationDraftLoads: Set<string>; translation: { ollamaModel: string }
}
export interface ActionElement {
  value: string; disabled: boolean; textContent: string | null
  classList: { contains: (name: string) => boolean; toggle: (name: string, force?: boolean) => boolean }
  requestSubmit: () => void; setSelectionRange: (start: number, end: number) => void
  focus: () => void; scrollIntoView: (options: ScrollIntoViewOptions) => void
  showModal: () => void; close: () => void
}
export interface ComposerActionServices {
  $: (selector: string) => ActionElement
  selectedStateKey: () => string
  composerDrafts: { value: (key: string) => string }
  setCurrentComposerValue: (text: string) => void
  hideComposerMenu: () => void
  latestAgentResponseText: () => string
  ...
  dispatchBackendRpc: (backend: string, method: string, params: Record<string, unknown>, timeoutMs: number) => Promise<unknown>
  sessionRefKey: (backend: string, id: string) => string
  routerRuntimeKey: (backend: string, id: string) => string
  persistMessageQueue: (key: string) => Promise<unknown>
  runNextQueuedMessage: (ref: SessionRef | null) => Promise<unknown>
  sessionRefFromKey: (key: string) => SessionRef | null
  pauseMessageQueue: (ref: SessionRef) => void
}
export function createComposerActions(state: ComposerActionsState, services: ComposerActionServices) {
  ...
  let editingQueuedMessage: { key: string; id: string } | null = null
...
async function draftContinueWithOllama(source: string) {
  ...
  if (!response.ok) throw new Error(typeof error.message === 'string' ? error.message : t('Local Ollama could not draft a continuation'))
  return String(result.prompt || '').trim()
}

async function draftContinueWithSessionModel(source: string, stateKey: string) {
...
function ensureContinuationBackend(backend: string, generation: number, stateKey: string) {
...
function openQueuedMessageEditor(id: string) {
  const key = selectedStateKey()
...
async function saveEditedQueuedMessage(event: { preventDefault(): void }) {
