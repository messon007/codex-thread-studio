...
export function createBackgroundSessions(state, services) {
    ...
    function scheduleOpenCodeLifecycleReconciliation() {
        if (openCodeLifecycleReconciliation)
        ...
        openCodeLifecycleReconciliation = new Promise((resolve) => {
            ...
            for (const id of staleRunningIds) {
                ...
                if (!model?.activeTurnId && lastTurn) {
                    handleQueuedTurnCompletion({ backend: 'opencode', id }, lastTurn.status || model.status || 'completed');
                }
            }
        }).catch((error) => {
    ...
    function handleInactiveCodexAppServerStatus(backend, message) {
        ...
        if ($('#connections-dialog').open)
            renderConnectionsDialog();
        if ($('#backend-dialog').open)
            renderBackendDialog();
    }
    ...
    function scheduleCodexLifecycleReconciliation() {
        scheduleStudioIdleWork(() => Promise.all(getBackendIds()
    ...
    async function reconcileCodexLifecycleBackend(backend) {
        if (!isCodexBackend(backend) || backend === state.backend)
            return;
        ...
        for (const entry of staleRunning) {
            ...
            await refreshOffscreenCodexHistoryAfterCompletion(backend, entry.id, entry.model, { advanceQueue: true });
        }
    }
    function refreshOffscreenCodexHistoryAfterCompletion(backend, threadId, sourceModel, { advanceQueue = false } = {}) {
        if (!threadId || backend === state.backend)
            return;
        const key = threadCatalogKey(backend, threadId);
        if (codexOffscreenHistoryRefreshes.has(key)) {
            codexOffscreenHistoryRefreshRetries.set(key, Boolean(codexOffscreenHistoryRefreshRetries.get(key) || advanceQueue));
        ...
        refresh = (async () => {
            ...
            if (advanceQueue && !model.activeTurnId && lastTurn) {
                handleQueuedTurnCompletion({ backend, id: threadId }, lastTurn.status || model.status || 'completed');
            }
            ...
            if (codexOffscreenHistoryRefreshes.get(key) === refresh)
                codexOffscreenHistoryRefreshes.delete(key);
            ...
            if (retryAdvanceQueue != null && backend !== state.backend) {
                queueMicrotask(() => {
                    ...
                    if (current)
                        refreshOffscreenCodexHistoryAfterCompletion(backend, threadId, current, { advanceQueue: retryAdvanceQueue });
                });
            }
        });
    ...
    function handleCodexLifecycleNotification(backend, message) {
        ...
        if (!event)
            return false;
        if (captureStructuredUtilityNotification(backend, message))
            return true;
        ...
        if (event.method === 'turn/completed' && threadId) {
            handleQueuedTurnCompletion({ backend, id: threadId }, String(message.params?.turn?.status || message.params?.status || 'completed'));
        }
        ...
        if (fullEventCoverage)
            ...
            markCachedModelUnvalidated(backend, targetModel);
        if (event.method === 'turn/completed') {
            ...
            notifyDesktop(t('Work completed'), threadTitle(completedThread || { name: t('Untitled session') }), { backend, id: threadId || targetModel.threadId });
        ...
        if (updateKind === 'full') {
            ...
            if (!turnId || !replaceRenderedTurn(turnId))
                renderTranscript();
        }
        renderComposerState();
        reportCodexLifecycleNotification(backend, message, {
    ...
    function handleOpenCodeServerEvent(event) {
        ...
        if (!payload?.type || payload.type === 'sync' || payload.type === 'server.heartbeat')
            return;
        ...
        if (repeatedTerminalLoop)
            abortRepeatedOpenCodeTerminalLoop(repeatedTerminalLoop);
        const eventThread = payload.properties?.info || { id: eventThreadId };
        if (hiddenUtilityThread('opencode', eventThread)) {
            if (eventThreadId)
                markUtilityThreadHidden('opencode', eventThreadId);
            return;
        }
        ...
        if (payload.type === 'session.deleted') {
            ...
            if (threadRouter.removeSession('opencode', deletedId))
                persistPreferences();
        ...
        if (completionSignal)
            scheduleOpenCodeStatusReconciliation(eventThreadId);
        ...
        if (completionSignal)
            sessionResources.invalidate('opencode', eventThreadId);
        if (!selectedTarget || targetModel !== state.model)
            return;
        if (update.kind === 'stream')
            queueStreamingItemPatch({ turnId: update.turnId, itemId: update.itemId });
        else if (update.kind === 'metadata') {
            renderComposerState();
        }
        else if (!update.turnId || !replaceRenderedTurn(update.turnId))
            renderTranscript();
        renderComposerState();
    }
        ...
        try {
            ...
            if (ref.backend === 'opencode') {
                ...
                if (historyEvents?.length)
                    replayOpenCodeEventsAfterHistory(model, historyEvents, ref.id, {
            ...
            if (historyEvents)
                endOpenCodeHistoryEventBuffer(ref.id, historyEvents);
        }
    }
