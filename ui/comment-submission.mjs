...
export function insertCommentPrompt(composerText, prompt) {
...
export class CommentSubmissionCoordinator {
    ...
    async submit(options) {
        ...
        try {
            ...
            if (remaining.length)
                store.annotationDrafts[key] = remaining;
            else
                delete store.annotationDrafts[key];
            if (store.annotationAdditional[key] === additional)
                delete store.annotationAdditional[key];
            options.persist(key);
            return 'sent';
        }
        finally {
