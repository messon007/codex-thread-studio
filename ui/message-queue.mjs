...
export function normalizeQueueDepth(value) {
    const depth = Math.trunc(Number(value));
    return depth >= 1 && depth <= MAX_MESSAGE_QUEUE_DEPTH ? depth : 1;
}
export function normalizeStoredMessageQueues(value, now = () => Date.now()) {
    const normalized = {};
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return normalized;
    for (const [key, entries] of Object.entries(value).slice(0, 2048)) {
        if (typeof key !== 'string' || !key.includes(':') || key.length > 320 || !Array.isArray(entries))
            continue;
        const messages = entries.slice(0, MAX_MESSAGE_QUEUE_DEPTH).flatMap((entry) => {
            ...
            const input = Array.isArray(entry.input)
                ? entry.input.slice(0, 32).filter((item) => isRecord(item) && item.type !== 'text')
                : [];
            if (!id || (!text && !input.length))
                return [];
...
export function completedQueueShouldAdvance(status, paused) {
