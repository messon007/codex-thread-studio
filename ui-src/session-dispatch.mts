import type { SessionRef, NormalizedSessionRef, SessionAdapter, SessionReadOptions, PreparationOptions, SessionTurnInput, SessionStartOptions, PreparedTurnRequest } from './backend-types.mjs'

export class SessionDispatchRegistry<ReadResult = unknown, StartResult = unknown> {
  adapters: Map<string, Readonly<SessionAdapter<ReadResult, StartResult>>>
  preparedSessions: Set<string>
  preparationLoads: Map<string, Promise<unknown>>
  preparationGenerations: Map<string, number>
  sessionPreparationGenerations: Map<string, number>
  ...
  register(backend: string, adapter: SessionAdapter<ReadResult, StartResult>) {
  ...
  read(ref: SessionRef, options: SessionReadOptions = {}) {
    const session = normalizeSessionRef(ref)
    return this.adapterFor(session).read(session, options)
  }

  prepareTurn(ref: SessionRef, options: PreparationOptions = {}): Promise<unknown> {
    const session = normalizeSessionRef(ref)
    ...
    const pending = this.preparationLoads.get(session.key)
  ...
  clearPrepared(backend: string | null = null) {
  ...
  startTurn(ref: SessionRef, input: SessionTurnInput, options: SessionStartOptions = {}) {
    const session = normalizeSessionRef(ref)
    return this.adapterFor(session).startTurn(session, input, options)
  }

  adapterFor(ref: SessionRef) {
    const session = normalizeSessionRef(ref)
...
export async function startTurnWithPreparation<T>({
  registry,
  ref,
  prepare,
  start,
  recoverThreadNotFound = false,
}: PreparedTurnRequest<T>): Promise<T> {
  await prepare()
  try {
    return await start()
  } catch (error) {
...
export function normalizeSessionRef(value: unknown): NormalizedSessionRef {
  const record = value && typeof value === 'object' ? value : {}
  const backend = normalizeBackend('backend' in record ? record.backend : undefined)
  const id = 'id' in record && typeof record.id === 'string' ? record.id.trim().slice(0, 256) : ''
  if (!backend || !id) throw new Error('Invalid session reference')
  return Object.freeze({ backend, id, key: `${backend}:${id}` })
}

function normalizeBackend(value: unknown) {
