...
test('session selection renders a valid cache before performing a first history load', () => {
  ...
  assert.ok(selectThread.indexOf('setNativeError(null)') < selectThread.indexOf('completeSessionSelection({'))
  assert.ok(selectThread.indexOf('renderTranscript()') < selectThread.indexOf('schedulePreferencesPersist()'))
  assert.ok(selectThread.indexOf('renderTranscript()') < selectThread.indexOf('await completeSessionSelection({'))
  assert.match(selectThread, /fresh: Boolean\(fresh\),[\s\S]*cached: Boolean\(cached\)/u)
  assert.match(selectThread, /resume: options => resumeThread\(id, options\)/u)
