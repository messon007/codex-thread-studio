...
const descriptors = new Map<string, BackendDescriptor>()

export const BACKEND_IDS: string[] = []
export const CODEX_BACKENDS: string[] = []

export function installBackendRegistry(entries: unknown = builtinDescriptors) {
...
export function backendDescriptor(backend: string) {
  const descriptor = descriptors.get(backend) || descriptors.get('codex')
  if (!descriptor) throw new Error('Backend registry has no Codex descriptor')
  return descriptor
}

export function isSupportedBackend(backend: string) {
  return descriptors.has(backend)
}

export function isCodexBackend(backend: string) {
  return descriptors.has(backend) && descriptors.get(backend)?.kind === 'codex'
}

export function defaultTurnOptions(backend: string) {
...
export function backendSearchAliases(backend: string) {
  const descriptor = backendDescriptor(backend)
  return `${descriptor.tag.toLowerCase()} ${descriptor.name.toLowerCase()} ${backend}`
}

function normalizeDescriptors(entries: unknown) {
  const values = Array.isArray(entries) ? entries : []
  const normalized: BackendDescriptor[] = []
  ...
  for (const builtin of builtinDescriptors) {
    ...
    if (!descriptor) throw new Error('Invalid built-in backend descriptor')
    ids.add(descriptor.id)
    normalized.push(descriptor)
  }
  ...
  return [codex, ...configured, opencode].filter((entry): entry is BackendDescriptor => Boolean(entry))
}

function normalizeDescriptor(value: unknown): BackendDescriptor | null {
