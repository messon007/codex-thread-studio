import type { OpenCodeEventModel, OpenCodeEvent, OpenCodeProperties, OpenCodePart, OpenCodeInfo, OpenCodeItem, OpenCodeTurn, OpenCodeReducerHelpers, OpenCodeError } from './opencode-event-types.mjs'
import type { OpenCodeSession, OpenCodeMessage, FetchMessagePage, FetchRootPage, OpenCodeHistoryThread, BufferedOpenCodeEvent, HistoryReplayOptions, OpenCodeProviderResult, LoopGuardState } from './opencode-history-types.mjs'
...
const openCodeReducerHelpers: OpenCodeReducerHelpers = {
...
export async function collectOpenCodeRootSessions(fetchPage: FetchRootPage, requestedPageSize = 100) {
...
export async function collectOpenCodeMessageHistory(fetchPage: FetchMessagePage, requestedPageSize = 500) {
  const pageSize = Math.max(1, Math.min(1_000, Number(requestedPageSize) || 500))
  const pages: OpenCodeMessage[][] = []
  const seenCursors = new Set()
...
export async function collectOpenCodeMessageTail(fetchPage: FetchMessagePage, anchorTurnIds: string[], requestedPageSize = 80) {
...
export async function fetchOpenCodeDirectoryStatuses(directories: string[], fetchStatus: (directory: string) => Promise<Record<string, unknown>>, requestedConcurrency = 6) {
...
export function mergeOpenCodeMessagePages(pages: OpenCodeMessage[][]) {
...
export function mergeOpenCodeThreadTail(model: OpenCodeEventModel | null, thread: OpenCodeHistoryThread, anchorTurnId: string) {
  ...
  const preserveMessageMetadata = <T,>(values: Record<string, T> | undefined) => Object.fromEntries(Object.entries(values || {})
...
export function sliceOpenCodeMessageTail(messages: OpenCodeMessage[], anchorTurnIds: Set<string> | string[]) {
...
export function normalizeOpenCodeSessions(sessions: OpenCodeSession[], statuses: Record<string, unknown> = {}) {
...
function newestOpenCodeAnchorIndex(messages: OpenCodeMessage[], anchors: Set<string>) {
...
export function openCodeThreadFromHistory(session: OpenCodeSession | null, messages: OpenCodeMessage[], status: unknown) {
  const turns: OpenCodeTurn[] = []
  const turnById = new Map<string, OpenCodeTurn>()
  const messageTurns: Record<string, string> = {}
  const messageRoles: Record<string, string | undefined> = {}
  const messageItems: Record<string, string[]> = {}
  const messageErrors: Record<string, { turnId: string; message: string }> = {}
  ...
  for (const message of Array.isArray(messages) ? messages : []) {
    ...
    if (info.role === 'user') {
      const studioUserParts: Record<string, unknown[]> = {}
    ...
    const turnId: string = info.parentID || current?.id || info.id || `assistant-${turns.length}`
...
export function applyOpenCodeEvent(model: OpenCodeEventModel, event: OpenCodeEvent, selectedSessionId: string | null) {
...
export function replayOpenCodeEventsAfterHistory(model: OpenCodeEventModel, bufferedEvents: ((OpenCodeEvent & { event?: undefined }) | BufferedOpenCodeEvent)[], selectedSessionId: string | null, {
  messageSnapshots = {},
  statusAfterSequence = -1,
  authoritativeStatus = null,
}: HistoryReplayOptions = {}) {
  const entries: BufferedOpenCodeEvent[] = (bufferedEvents || []).map((entry) => entry?.event
    ? entry
    : { event: entry, sequence: Number.POSITIVE_INFINITY })
  const ambiguousDeltas = new Map<string, OpenCodeEvent>()
  const snapshotForMessage = (messageId: unknown) => messageSnapshots?.[String(messageId || '')]
    || { afterSequence: -1, ambiguousThroughSequence: -1 }
  const flushAmbiguousDeltas = (matches: (properties: OpenCodeProperties) => boolean = () => true) => {
  ...
  const discardAmbiguousDeltas = (matches: (properties: OpenCodeProperties) => boolean) => {
    for (const [key, event] of ambiguousDeltas) {
  ...
  const applyStatusSnapshot = () => {
    ...
    if (authoritativeStatus === null) return
...
function applyOpenCodeStatusSnapshot(model: OpenCodeEventModel, status: string) {
  model.status = status
  if (status === 'running') {
...
function mergeAmbiguousOpenCodeDelta(model: OpenCodeEventModel, event: OpenCodeEvent, selectedSessionId: string | null) {
...
export function normalizeOpenCodeStatus(status: unknown) {
...
export function openCodeModelList(providerResult: OpenCodeProviderResult | null) {
...
export function splitOpenCodeModel(value: unknown) {
  const [providerID, ...model] = String(value || '').split('/')
  return providerID && model.length ? { providerID, modelID: model.join('/') } : null
}

export function openCodeCommandUserId(result: OpenCodeMessage | null) {
  const info = result?.info || {}
  return String(info.role === 'user' ? info.id || '' : info.parentID || '')
}

export function openCodeCommandTurn(result: OpenCodeMessage | null, input: unknown[] = []) {
  const info = result?.info || {}
  const userId = openCodeCommandUserId(result)
  if (!userId) return null
  const items: OpenCodeItem[] = [{ id: userId, type: 'userMessage', content: input }]
...
export function selectOpenCodeStartedUserMessage(messages: OpenCodeMessage[], { baselineIds = [], expectedText = '' }: { baselineIds?: string[]; expectedText?: string } = {}) {
...
export function createOpenCodeLoopGuard({ terminalLimit = 3 } = {}) {
  ...
  function clear(sessionId: unknown) {
    sessions.delete(String(sessionId || ''))
  }

  function observe(event: OpenCodeEvent) {
    const payload = event?.payload || event || {}
    const properties = payload.properties || {}
...
function userContentFromPart(part: OpenCodePart) {
...
function upsertOpenCodeUserPart(turn: OpenCodeTurn, part: OpenCodePart) {
  const additions = userContentFromPart(part)
...
function structuredOutputItem(info: OpenCodeInfo): OpenCodeItem {
  return {
...
function openCodePartToItem(part: OpenCodePart, role: string | undefined): OpenCodeItem | null {
...
function toolStatus(status: string | undefined) {
  const statuses: Record<string, string> = { pending: 'inProgress', running: 'inProgress', completed: 'completed', error: 'failed' }
  return statuses[String(status)] || status || 'inProgress'
}

function ensureTurn(model: OpenCodeEventModel, id: string | null | undefined): OpenCodeTurn {
...
function upsertItem(turn: OpenCodeTurn, item: OpenCodeItem | null) {
  if (!item) return
...
function rememberMessageItem(messageItems: Record<string, string[]>, messageId: string | undefined, itemId: string | undefined) {
...
function forgetMessageItem(messageItems: Record<string, string[]> | undefined, messageId: string, itemId: string) {
  const messageKey = String(messageId || '')
...
function refreshTurnMessageError(model: OpenCodeEventModel, turn: OpenCodeTurn) {
  let error = null
...
function mergeOpenCodeStreamText(historyValue: unknown, liveValue: unknown) {
...
function errorText(error: OpenCodeError | OpenCodeProperties | null | undefined) {
