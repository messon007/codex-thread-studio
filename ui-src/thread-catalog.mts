import type { CatalogThread, CatalogEntry, BackendCatalogs, CatalogFilter, IgnoreRule } from './catalog-search-types.mjs'
...
export function compactSidebarText(value: unknown, limit = SIDEBAR_TEXT_LIMIT) {
...
export function syncCatalogSelection(rows: Iterable<HTMLElement> | null, backend: string, selectedId: string | null) {
...
export function threadCatalogKey(backend: string, id: string) {
  return `${backend}:${id}`
}

export function isActiveCatalogThread(thread: CatalogThread | null | undefined) {
  return activeStatuses.has((typeof thread?.status === 'object' ? thread.status?.type : thread?.status) || '')
}

export function catalogEntries(catalogs: BackendCatalogs = {}) {
  return BACKEND_IDS.flatMap((backend) =>
    (catalogs[backend] || []).map((thread) => ({ backend, thread })),
  )
}

export function catalogCountsWithAttention(catalogs: BackendCatalogs = {}, attention: Set<string> = new Set(), hiddenDirectories: string[] = [], ignorePatterns: string[] = []) {
...
export function groupCatalogEntries(entries: CatalogEntry[] = []) {
...
function compareCatalogEntriesByActivity(left: CatalogEntry, right: CatalogEntry) {
...
export function catalogActivityTimestamp(thread: CatalogThread | null | undefined) {
  return Math.max(
...
export function partitionPinnedCatalogEntries(entries: CatalogEntry[] = [], pinned: Set<string> = new Set(), { order = 'pin' }: { order?: 'pin' | 'activity' } = {}) {
  const pinOrder = new Map([...pinned].map((key, index) => [key, index]))
  const pinnedEntries: CatalogEntry[] = []
  const regularEntries: CatalogEntry[] = []
  for (const entry of entries) {
...
export function filterCatalogEntries(catalogs: BackendCatalogs, {
  filter = 'all',
  search = '',
  attention = new Set(),
  hiddenDirectories = [],
...
export function normalizeHiddenSessionDirectories(values: unknown = []) {
  if (!Array.isArray(values)) return []
  return [...new Set(values.map(normalizeDirectoryPath).filter(Boolean))]
}

export function isSessionDirectoryHidden(directory: unknown, hiddenDirectories: string[] = [], ignorePatterns: string[] = []) {
...
export function normalizeSessionDirectoryIgnore(patterns: unknown = []) {
  if (!Array.isArray(patterns)) return []
  return patterns.map(parseIgnoreRule).filter((rule): rule is IgnoreRule => Boolean(rule))
}

function parseIgnoreRule(value: unknown): IgnoreRule | null {
...
function globPatternSource(pattern: string) {
  let source = ''
...
function normalizeDirectoryPath(value: unknown) {
...
function isWindowsDirectory(path: string) {
  return /^[a-z]:\//iu.test(path) || path.startsWith('//')
}

export function catalogTimestamp(value: unknown) {
...
export function isCatalogCacheFresh(updatedAt: unknown, validatedAt: unknown, { coarse = false } = {}) {
