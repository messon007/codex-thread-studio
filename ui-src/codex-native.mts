import type { CodexViewModel, CodexTurn, CodexItem, IncomingCodexTurn, IncomingCodexItem, CodexNotification } from './codex-types.mjs'

import { markTranscriptModelChanged } from './model-revision.mjs'

export function createCodexViewModel(): CodexViewModel {
  return {
    threadId: null,
    turns: [],
    activeTurnId: null,
...
export function beginOptimisticCodexTurn(model: CodexViewModel, { clientUserMessageId, input }: { clientUserMessageId?: string; input?: unknown[] }) {
...
export function reconcileOptimisticCodexTurn(model: CodexViewModel, optimisticTurnId: string, incomingTurn: IncomingCodexTurn | null) {
...
export function rollbackOptimisticCodexTurn(model: CodexViewModel, optimisticTurnId: string) {
...
export function applyCodexNotification(model: CodexViewModel, message: CodexNotification) {
  const handled = applyCodexNotificationInternal(model, message)
  if (handled) markTranscriptModelChanged(model)
  return handled
}

function applyCodexNotificationInternal(model: CodexViewModel, message: CodexNotification) {
...
export function selectedThreadStatusChange(message: CodexNotification, selectedThreadId: string | null) {
...
export function resolveCodexApproval(model: CodexViewModel, requestId: string | number) {
  model.approvals = model.approvals.filter((approval) => String(approval.id) !== String(requestId))
}

export function resolveCodexInteraction(model: CodexViewModel, requestId: string | number) {
...
function normalizeThreadStatus(status: { type?: string } | null | undefined) {
  if (status?.type === 'active') return 'running'
  if (status?.type === 'systemError') return 'failed'
  return status?.type || 'idle'
}

function upsertTurn(model: CodexViewModel, incoming: IncomingCodexTurn): CodexTurn {
...
function mergeTurnItems(current: CodexItem[], incoming: CodexItem[] | undefined) {
...
function ensureTurn(model: CodexViewModel, turnId: string | null | undefined): CodexTurn {
...
function upsertItem(turn: { items: CodexItem[] }, incoming: IncomingCodexItem): CodexItem {
...
function sameUserMessage(left: IncomingCodexItem | undefined, right: IncomingCodexItem | undefined) {
...
function sameUserContent(left: unknown, right: unknown) {
...
function ensureItem(model: CodexViewModel, turnId: string | undefined, itemId: string | undefined, type: string): CodexItem {
