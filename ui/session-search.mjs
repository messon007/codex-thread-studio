...
export function isUnsupportedOccurrenceSearchError(error) {
    if (Number(record(error).code) === -32601)
        return true;
...
export function localSessionOccurrences(model, searchTerm, { includeMessages = true, includeActivity = true } = {}) {
    ...
    turns.forEach((turn, turnIndex) => {
        ...
        items.forEach((item, itemIndex) => {
            ...
            if (type === 'activity' ? !includeActivity : !includeMessages)
                return;
...
export function mergeSessionOccurrences(remote = [], local = []) {
    ...
    for (const entry of local) {
        if (entry.type !== 'activity' && remoteItems.has(`${entry.turnId}:${entry.itemId}`))
            continue;
        merged.push(entry);
    }
...
function occurrenceType(item, itemIndex, finalAssistantIndex) {
    if (item?.type === 'userMessage')
        return 'user';
    if ((item?.type === 'agentMessage' || item?.type === 'plan') && itemIndex === finalAssistantIndex)
        return 'assistant';
    return 'activity';
}
...
function boundedValueText(value) {
    ...
    if (typeof value === 'string')
        return value.slice(0, MAX_SEARCHABLE_TEXT);
    try {
        return JSON.stringify(value).slice(0, MAX_SEARCHABLE_TEXT);
    }
    catch {
        return String(value).slice(0, MAX_SEARCHABLE_TEXT);
    }
}
