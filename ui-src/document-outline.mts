import type { OutlineItem, InlineToken, PdfOutlineEntry, PdfOutlineDocument } from './outline-types.mjs'
import { marked } from './vendor/marked.esm.js'

const MAX_OUTLINE_ITEMS = 2_000
const MAX_LABEL_LENGTH = 512

export function extractMarkdownOutline(source: unknown, parsedTokens: InlineToken[] | null = null) {
...
export function extractHtmlOutline(source: unknown) {
  const value = String(source || '')
  const items: OutlineItem[] = []
  const occurrences = new Map()
  const blockedRanges: [number, number][] = [...value.matchAll(/<!--[\s\S]*?-->|<(script|style|template|noscript)\b[^>]*>[\s\S]*?<\/\1\s*>/giu)]
...
export function normalizeDocumentOutline(items: OutlineItem[] = []) {
...
export function filterDocumentOutline(items: OutlineItem[], query: unknown) {
...
export async function extractPdfOutline(document: PdfOutlineDocument) {
  const source = await document.getOutline().catch(() => null)
  if (!Array.isArray(source) || !source.length) return []
  const result: OutlineItem[] = []
  async function append(items: PdfOutlineEntry[] | undefined, depth: number): Promise<void> {
    for (const item of items || []) {
      ...
      if (typeof reference === 'number' && Number.isInteger(reference)) page = reference + 1
...
function withHierarchy(items: OutlineItem[]) {
...
function inlineTokenText(tokens: InlineToken[] | undefined): string {
...
function outlineSlug(value: unknown) {
...
function lineNumberAt(value: unknown, offset: number) {
