...
export function createTranscriptDom({ maxBytes = 4 * 1024 * 1024 } = {}) {
  let context = ''
  let entries = new Map<string, { html: string; nodes: ChildNode[] }>()
  return {
    invalidate(id: string) { entries.delete(String(id)) },
    render(container: HTMLElement, key: string, chunks: { id: string; html: string }[]) {
      if (context !== key) entries.clear()
      context = key
      const next = new Map<string, { html: string; nodes: ChildNode[] }>()
