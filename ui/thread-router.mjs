...
export function normalizeThreadRouter(value) {
    ...
    for (const [backend, id] of Object.entries(record(source.controllers))) {
        ...
        if (key && controllerId)
            controllers[key] = controllerId;
    }
    const legacyThreadId = cleanText(source.threadId, 256);
    if (legacyThreadId && !controllers.codex)
        controllers.codex = legacyThreadId;
    ...
    const appendFallback = (rawKey, rawCondition) => {
        ...
        if (!key || controllerKeys.has(key) || seenFallbacks.has(key) || fallbacks.length >= 3)
            return;
    ...
    for (const entry of Array.isArray(source.fallbacks) ? source.fallbacks : []) {
        if (entry && typeof entry === 'object')
            appendFallback(entry.sessionKey, entry.condition);
    }
    if (!fallbacks.length) {
...
export function migrateLegacyResponsibilities(openingMessages, router) {
    ...
    for (const [rawKey, raw] of Object.entries(record(record(router).responsibilities)).slice(0, 2048)) {
        ...
        if (!key || controllerKeys.has(key) || !description || messages[key]?.responsibility)
            continue;
...
export function managedRouterThread(threads, routerId, routerWorkspace) {
    ...
    if (!id || !cwd)
        return null;
    return (Array.isArray(threads) ? threads : []).find((thread) => String(thread?.id || '') === id && String(thread?.cwd || '') === cwd) || null;
}
export function recoverManagedRouterCatalog(threads, routerId, routerWorkspace, recovered) {
    ...
    if (managedRouterThread(catalog, routerId, routerWorkspace))
        return catalog;
    if (!managedRouterThread([recovered], routerId, routerWorkspace))
        return catalog;
...
export function parseRouterDecision(value, candidateKeys = [], { allowLegacyClarify = false } = {}) {
    ...
    if (!raw || typeof raw !== 'object')
        throw new Error('Router did not return an object');
    ...
    if (decision.action === 'dispatch') {
        if (!allowed.includes(decision.targetSessionKey))
            throw new Error('Router selected a session outside the configured catalog');
        if (!decision.forwardedPrompt)
            throw new Error('Router returned an empty forwarded prompt');
    }
    else {
        decision.targetSessionKey = '';
        decision.forwardedPrompt = '';
        if (!decision.message)
            throw new Error('Router did not provide a clarification question');
    }
    return decision;
}
...
export function routerDecisionForTurn(turn, candidateKeys = []) {
    ...
    if (!text)
        return null;
    try {
        return parseRouterDecision(text, candidateKeys, { allowLegacyClarify: true });
    }
    catch {
        return null;
    }
...
function parseJson(value) {
    ...
    if (!text)
        throw new Error('Router returned no result');
