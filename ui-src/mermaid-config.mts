type Style = 'auto' | 'classic' | 'neo' | 'handDrawn' | 'document'
type Density = 'compact' | 'standard' | 'loose'
type Curve = 'rounded' | 'linear' | 'step' | 'basis'
type Layout = 'auto' | 'dagre' | 'elk'
...
export function normalizeMermaidPreferences(value: unknown) {
  const source = value && typeof value === 'object' ? value as Record<string, unknown> : {}
  const fontSize = Number(source.fontSize)
  return {
    style: typeof source.style === 'string' && allowed.style.has(source.style) ? source.style as Style : MERMAID_PREFERENCES_DEFAULTS.style,
    density: typeof source.density === 'string' && allowed.density.has(source.density) ? source.density as Density : MERMAID_PREFERENCES_DEFAULTS.density,
    curve: typeof source.curve === 'string' && allowed.curve.has(source.curve) ? source.curve as Curve : MERMAID_PREFERENCES_DEFAULTS.curve,
    layout: typeof source.layout === 'string' && allowed.layout.has(source.layout) ? source.layout as Layout : MERMAID_PREFERENCES_DEFAULTS.layout,
...
export function mermaidInitializeConfig(preferences: unknown, { dark = false, fontFamily = '' } = {}) {
