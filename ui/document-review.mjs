...
const STRUCTURED_TEXT_EXTENSIONS = Object.freeze({
...
const STRUCTURED_TEXT_NAMES = Object.freeze({
...
export function resolveMarkdownFileLink(value, documentPath = '') {
    let raw = String(value || '').trim();
    if (!raw || raw.startsWith('#') || /^https?:\/\//iu.test(raw))
        return null;
    const fragmentLocation = raw.match(/#L(\d+)(?:C(\d+))?$/iu);
    if (fragmentLocation)
        raw = raw.slice(0, fragmentLocation.index);
    else
        raw = raw.replace(/[?#].*$/u, '');
    try {
...
export function normalizeAnnotationTarget(value = {}) {
...
export function snapshotAnnotationSelection(value) {
...
export function locateQuote(content, quote, fromOffset = 0) {
    ...
    if (!selected)
        return { startOffset: null, endOffset: null, ambiguous: false };
...
export function lineRangeForTarget(value, content = null) {
...
export function fileAnnotationAnchor(value, content = null) {
    const target = record(value);
    if (target?.kind !== 'fileRange' || !target.filePath)
        return '';
...
export function structuredTextPreviewKind(path) {
    const normalized = String(path || '').replaceAll('\\', '/');
    const name = normalized.split('/').filter(Boolean).pop()?.toLowerCase() || '';
    if (!name)
        return null;
    if (name === '.env' || name.startsWith('.env.'))
        return { language: 'config', label: 'ENV' };
...
export function renderStructuredTextPreview(path, value) {
...
export function structuredPreviewSourceRange(sourceMap, startOffset, endOffset) {
    const start = finiteOffset(startOffset);
    const end = finiteOffset(endOffset);
    if (start == null || end == null || end <= start)
        return null;
    if (!(sourceMap instanceof Int32Array))
...
function formatStructuredPreviewSource(format, source) {
    if (!source.trim())
        return null;
    if (format.language === 'json')
        return formatJsonPreviewSource(source);
    if (format.language === 'xml')
        return formatXmlPreviewSource(source);
    if (format.language === 'toml')
        return formatAssignmentPreviewSource(source, '=');
    if (format.language === 'config')
        return formatAssignmentPreviewSource(source, '=:');
...
function createMappedPreviewBuilder(source) {
    const parts = [];
    const sourceOffsets = [];
    let length = 0;
    let overflow = false;
...
function formatJsonPreviewSource(source) {
    ...
    for (let index = 0; index < tokens.length; index += 1) {
        ...
        if (token.kind === 'open') {
            if (depth === 0 && previous && !atLineStart)
                newline();
            appendToken(token);
            stack.push(token.value);
            depth += 1;
            if (!(next?.kind === 'close' && closesJsonToken(token.value, next.value)))
                newline();
        }
        else if (token.kind === 'close') {
            const opening = stack.pop();
            if (!opening || !closesJsonToken(opening, token.value))
                return null;
            depth = Math.max(0, depth - 1);
            if (!(previous?.kind === 'open' && closesJsonToken(previous.value, token.value)))
                newline();
            appendToken(token);
        }
            ...
            if (!atLineStart)
                space();
            appendToken(token);
            if (next && next.kind !== 'comma' && next.kind !== 'colon' && next.kind !== 'close')
                space();
        }
        else {
            if (depth === 0 && previous && !atLineStart)
    ...
    if (stack.length)
        return null;
    if (/\r?\n$/u.test(source) && !atLineStart)
        newline();
    return builder.finish();
}
function tokenizeJsonPreviewSource(source) {
    const tokens = [];
    for (let index = 0; index < source.length;) {
        ...
        while (end < source.length) {
            ...
            if (/\s/u.test(next) || /[{}\[\],:'"]/u.test(next))
                break;
            if (next === '/' && (source[end + 1] === '/' || source[end + 1] === '*'))
                break;
            end += 1;
        }
        tokens.push({ kind: 'value', start: index, end, value: source.slice(index, end) });
...
function quotedTokenEnd(source, start, quote) {
...
function closesJsonToken(opening, closing) {
    return (opening === '{' && closing === '}') || (opening === '[' && closing === ']');
}
function formatXmlPreviewSource(source) {
    const tokens = tokenizeXmlPreviewSource(source);
    if (!tokens?.length || !xmlPreviewStructureIsSafe(tokens))
        return null;
...
function tokenizeXmlPreviewSource(source) {
    const tokens = [];
    for (let index = 0; index < source.length;) {
        if (source[index] !== '<') {
...
function xmlMarkupEnd(source, start) {
...
function xmlPreviewStructureIsSafe(tokens) {
    const stack = [];
    for (const token of tokens) {
        if (token.kind === 'start') {
            ...
            if (!current || current.name !== token.name || (current.hasChild && current.hasText))
                return false;
        }
    }
    return stack.length === 0;
}
function formatAssignmentPreviewSource(source, delimiters) {
    const builder = createMappedPreviewBuilder(source);
    let cursor = 0;
    let inTomlMultiline = '';
    while (cursor < source.length) {
        ...
        if (delimiter >= 0) {
            ...
            while (leftEnd > 0 && /[\t ]/u.test(line[leftEnd - 1]))
                leftEnd -= 1;
            let rightStart = delimiter + 1;
            while (rightStart < line.length && /[\t ]/u.test(line[rightStart]))
                rightStart += 1;
        ...
        if (newline >= 0)
            builder.appendSource(contentEnd, newline + 1);
        if (inTomlMultiline && triple === inTomlMultiline)
            inTomlMultiline = '';
        else if (!inTomlMultiline && triple)
            inTomlMultiline = triple;
        cursor = newline < 0 ? source.length : newline + 1;
    }
    return builder.finish();
}
function assignmentDelimiterIndex(line, delimiters) {
    let quote = '';
    let escaped = false;
    let colon = -1;
    for (let index = 0; index < line.length; index += 1) {
        ...
        if (character === '=' && delimiters.includes(character))
            return index;
        if (character === ':' && delimiters.includes(character) && colon < 0)
            colon = index;
        if (character === '#' || character === ';')
            return -1;
    }
...
function tomlMultilineDelimiter(line, active) {
    const candidates = active ? [active] : ['"""', "'''"];
    for (const candidate of candidates) {
        const first = line.indexOf(candidate);
        if (first < 0)
            continue;
        if (active || line.indexOf(candidate, first + candidate.length) < 0)
            return candidate;
    }
    return '';
}
function renderJsonTokens(source) {
    return renderTokenMatches(source, JSON_TOKEN_PATTERN, (token, index) => {
        ...
        if (/^[{}\[\],:]$/u.test(token))
            return 'punctuation';
        if (/^(?:true|false|null|Infinity|NaN|undefined)$/u.test(token))
            return 'literal';
        if (/^-?\d/u.test(token))
            return 'number';
        return 'key';
...
function renderYamlTokens(source) {
    return renderTokenMatches(source, YAML_TOKEN_PATTERN, (token, index) => {
        ...
        if (/^[&*!]/u.test(trimmed))
            return 'symbol';
        if (/^(?:true|false|null|yes|no|on|off|~)$/iu.test(trimmed))
            return 'literal';
        if (/^-?\d/u.test(trimmed))
            return 'number';
        if (/^[{}\[\],:|>]$/u.test(trimmed))
...
function renderXmlTokens(source) {
    return renderTokenMatches(source, XML_TOKEN_PATTERN, (token) => {
        if (token.startsWith('<!--'))
            return 'comment';
        if (token.startsWith('<![CDATA['))
            return 'cdata';
        if (token.startsWith('<?') || /^<!DOCTYPE/iu.test(token))
            return 'document';
        if (token.startsWith('&'))
            return 'entity';
        return 'tag';
...
function renderTomlTokens(source) {
    return renderTokenMatches(source, TOML_TOKEN_PATTERN, (token, index) => {
        ...
        if (/^\s*=/u.test(source.slice(index + token.length)))
            return 'key';
...
function renderConfigTokens(source) {
    return renderTokenMatches(source, CONFIG_TOKEN_PATTERN, (token, index) => {
        ...
        if (/^\s*[=:]/u.test(source.slice(index + token.length)))
            return 'key';
        if (/^(?:true|false|yes|no|on|off|null|none)$/iu.test(trimmed))
            return 'literal';
        if (/^-?\d/u.test(trimmed))
            return 'number';
        return 'punctuation';
...
function renderTokenMatches(source, pattern, tokenClass) {
    pattern.lastIndex = 0;
    let cursor = 0;
    let html = '';
