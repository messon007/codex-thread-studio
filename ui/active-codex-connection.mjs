...
export function createActiveCodexConnection(state, services) {
    ...
    function handleAppServerMessage(message) {
            ...
            if (status === 'ready') {
                ...
                if (firstReady) {
                    ...
                    const selectionLoad = loadThreads({ applyCachedEnvironment: !appServerRestarted }).then(async () => {
                        ...
                        if (activeHistory) {
                            try {
                                ...
                                console.warn('Selected history load failed before reconnect recovery', error);
                            }
                        }
                        if (state.backend !== backend
                            || state.socketGeneration !== socketGeneration
                        ...
                        if (state.threadModels.get(selectedKey) !== cachedModelsBeforeCatalog.get(selectedKey))
                            return;
                        ...
                        if (!appServerRestarted && freshThreadModel(backend, selectedId, { reconnectValidation: true }))
                            return;
                    ...
                    selectionLoad
                        ...
                        .finally(() => {
                        if (backendSelectionLoads.get(backend) === selectionLoad)
                            backendSelectionLoads.delete(backend);
                    });
                }
            }
        ...
        if (message.method === 'thread/started' && message.params?.thread) {
            if (message.params.thread.ephemeral || state.hiddenCodexThreads.has(sessionRefKey(state.backend, message.params.thread.id)))
                return;
        ...
        if (message.method === 'thread/name/updated') {
            ...
            if (thread)
                thread.name = message.params.threadName;
            renderThreadList();
            renderWorkspace();
            return;
        }
        if ((message.method === 'thread/archived' || message.method === 'thread/deleted')
            && hiddenUtilityThread(state.backend, { id: message.params?.threadId }))
            return;
        if (message.method === 'thread/archived' || message.method === 'thread/deleted') {
            const backend = state.backend;
        ...
        if (message.method === 'item/completed') {
            sessionResources.invalidate(backend, message.params?.threadId || targetModel.threadId || state.selectedId);
        }
        if (targetModel !== state.model) {
            reportCodexLifecycleNotification(backend, message, {
                targetModel,
        ...
        if (updateKind === 'stream')
            queueStreamingItemPatch(message.params);
        else if (updateKind === 'item')
            replaceCompletedItem(message.params);
    ...
    async function resynchronizeSelectedThreadAfterLag({ backend, threadId, socketGeneration, skipped }) {
        ...
        if (activeHistory) {
            try {
                ...
                console.warn('Selected history load failed before lag recovery', error);
            }
        }
        if (state.backend !== backend
