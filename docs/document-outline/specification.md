...
目录面板位于 Document Header 下方、Document Footer 上方，是文档画布内部的左侧抽屉。它不创建第二个右栏，也不改变用户拖拽得到的共享右栏宽度。

- 宽度：240 px；最大不超过 Document Viewer 内容宽度的 42%。
- 面板标题行：`目录`、章节数量、关闭图标。
- 筛选框：`筛选章节…`，只过滤显示项，不改变正文。
- 列表：扁平渲染的层级树；每级缩进 14 px，视觉缩进最多四级，语义深度完整保留。
...
当 Document Viewer 内容宽度不小于 680 px 时，目录采用 docked 模式，正文让出 240 px，并保持原有正文最大宽度和阅读留白。

当内容宽度小于 680 px 时，目录采用 overlay 模式：覆盖正文左侧，宽度为 `min(280px, 78%)`，带轻量阴影和透明遮罩。点击遮罩、目录项或关闭按钮后关闭。这样不会把 Markdown 正文、PDF 页面或编辑器压缩成不可用的窄列。

目录开合不能改变 `--right-rail-width`，也不能触发共享右栏宽度持久化。
...
- 打开新文档时，保留用户最近一次“目录打开/关闭”偏好；若新文档无目录则自然隐藏。
...
4. 目标章节短暂显示品牌色定位提示；
5. overlay 模式自动关闭，docked 模式保持打开。
...
Document Shell 只管理抽屉、筛选、active 状态与响应式布局。文档格式通过统一接口提供结构和导航：
...
- `state.artifactOutlineOpen`：用户最后一次开合偏好，可随 Studio preferences 保存。
...
8. 宽屏目录 docked，窄屏目录 overlay；两种模式都不改变共享右栏宽度。
9. 目录、搜索、刷新、关闭图标均为 32 × 32 px，Header 分隔线仍与中间会话 Header 精确对齐。
10. 编辑标题后目录在 250 ms 左右刷新，保存、刷新、关闭无 listener 泄漏。

## 10. UI 决策摘要

推荐采用草图中的方案：一个统一 Header 图标、一块文档内部左侧目录面板、宽屏 docked / 窄屏 overlay。它保留了正文区域的连续性，也避免把目录做成新的共享右栏工具；对用户而言，目录始终是“当前文档的一部分”。
