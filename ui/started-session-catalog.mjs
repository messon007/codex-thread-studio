...
export function createStartedSessionCatalog(effects) {
    ...
    function remember(backend, thread, operation) {
        if (!thread?.id)
            throw new Error(effects.missingId());
    ...
    function forget(backend, threadId, reason) {
        if (!threadId)
            return false;
    ...
    function reconcile(backend, threads) {
        ...
        const startedThreads = started.map(entry => {
            ...
            if (current)
                entry.thread = { ...entry.thread, ...current, turns: undefined };
            return entry.thread;
        });
        ...
        for (const threadId of result.retainedIds) {
            ...
            if (!entry || entry.catalogMissLogged)
                continue;
        ...
        for (const threadId of result.confirmedIds)
            forget(backend, threadId, 'catalog-confirmed');
        return result.threads;
    }
    function schedule(backend, threadId, delay = 800) {
        const key = effects.key(backend, threadId);
        if (!pending.has(key))
            return;
        clearTimeout(timers.get(key));
    ...
    function clearBackend(backend, reason) {
        for (const entry of entries(backend))
            forget(backend, entry.thread.id, reason);
    }
