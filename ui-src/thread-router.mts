import type { RouterConfiguration, RouterThread, RouterOpening, RouterCandidate, RouterDecision, RouterTurn } from './router-types.mjs'
...
export function sessionRefKey(backend: unknown, id: unknown) {
...
export function parseSessionRefKey(value: unknown) {
...
export function normalizeThreadRouter(value: unknown): RouterConfiguration {
  const source = record(value)
  const controllers: Record<string, string> = {}
  for (const [backend, id] of Object.entries(record(source.controllers))) {
  ...
  const fallbacks: RouterConfiguration['fallbacks'] = []
  const seenFallbacks = new Set()
  const appendFallback = (rawKey: unknown, rawCondition: unknown) => {
  ...
  if (!fallbacks.length) {
    for (const [rawKey, raw] of Object.entries(record(source.responsibilities))) {
      if (record(raw).fallback === 'fallback' || ['learning', 'general'].includes(String(record(raw).fallback))) {
...
export function migrateLegacyResponsibilities(openingMessages: Record<string, RouterOpening> | null | undefined, router: unknown) {
  ...
  for (const [rawKey, raw] of Object.entries(record(record(router).responsibilities)).slice(0, 2048)) {
    const parsed = parseSessionRefKey(rawKey)
    const key = parsed?.key || sessionRefKey('codex', rawKey)
    const description = cleanText(record(raw).description, 4096)
...
export function routerControllerRef(config: unknown, backend: string | null = null) {
...
export function isRouterSession(config: unknown, backend: string, id: unknown) {
  return routerControllerRef(config, backend)?.id === String(id || '')
}

export function catalogsWithSingleRouter<T extends RouterThread>(config: unknown, catalogs: Record<string, T[]>) {
  const normalized = normalizeThreadRouter(config)
...
export function managedRouterThread<T extends RouterThread>(threads: readonly T[], routerId: unknown, routerWorkspace: unknown) {
...
export function recoverManagedRouterCatalog<T extends RouterThread>(threads: readonly T[], routerId: unknown, routerWorkspace: unknown, recovered: T) {
...
export function shouldCreateManagedRouter(routerId: unknown) {
  return !cleanText(routerId, 256)
}

export function routerDecisionSchema(candidateKeys: readonly (string | { key: string })[] = []) {
...
export function routerCandidates(config: unknown, catalogs: Record<string, RouterThread[]> | RouterThread[], openingMessages: Record<string, RouterOpening> = {}): RouterCandidate[] {
  const normalized = normalizeThreadRouter(config)
  const controllerKeys = new Set(Object.entries(normalized.controllers).map(([backend, id]) => sessionRefKey(backend, id)))
...
export function routerDeveloperInstructions(candidates: readonly RouterCandidate[]) {
...
export function routerApplicationContext(candidates: readonly RouterCandidate[], responseContext = '') {
  return {
    [ROUTER_CONTEXT_KEY]: {
      kind: 'application',
...
export function parseRouterDecision(value: unknown, candidateKeys: readonly string[] = [], { allowLegacyClarify = true } = {}): RouterDecision {
  const raw: unknown = parseJson(value)
  const parsed = record(raw)
  if (!raw || typeof raw !== 'object') throw new Error('Router did not return an object')
...
export function finalAgentText(turn: RouterTurn | null | undefined) {
  return (turn?.items || [])
...
export function routerDecisionForTurn(turn: RouterTurn | null | undefined, candidateKeys: readonly string[] = []) {
...
function cleanBackend(value: unknown) {
