...
export function extractHtmlOutline(source) {
    ...
    while ((match = headingPattern.exec(value)) && items.length < MAX_OUTLINE_ITEMS) {
        if (blockedRanges.some(([start, end]) => match.index >= start && match.index < end))
            continue;
...
export function filterDocumentOutline(items, query) {
    ...
    if (!needle)
        return list.map((item) => ({ ...item, contextOnly: false }));
...
export async function extractPdfOutline(document) {
    ...
    async function append(items, depth) {
        for (const item of items || []) {
            if (result.length >= MAX_OUTLINE_ITEMS)
                return;
            ...
            if (typeof reference === 'number' && Number.isInteger(reference))
                ...
                page = await document.getPageIndex(reference).then((index) => index + 1).catch(() => null);
            if (page != null) {
                result.push({
...
function withHierarchy(items) {
    ...
    return items.map((item) => {
        ...
        while (ancestors.length && ancestors.at(-1).depth >= depth)
            ancestors.pop();
