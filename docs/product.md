...
14. Git Review: inspect the selected session repository without shell interpolation; filter all, staged, and unstaged code/Markdown/CSV/text changes; browse paginated commit history and each commit's changed files; read bounded line-numbered unified diffs; and stage or unstage a selected current file. Historical inspection is read-only and destructive discard is not exposed.
...
26. A fresh Studio process starts with no retained Thread history cache, opens the configured Thread Router controller, and loads other sessions only when the user selects them.
...
35. Git status handles spaces, untracked files, staged/unstaged overlap, renames, and conflicts through NUL-delimited porcelain output. Current and historical Diff requests accept only validated root-relative paths from the corresponding change set, remain bounded, and never pass a command through a shell. Commit history is paginated, uses full validated object IDs, and compares merge commits to their first parent.
