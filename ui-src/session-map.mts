import type { SessionMap, SessionMapItem, SessionMapWorker, MapState, MapStructure, AssistantMapState, AssistantMapOperation, ParsedSessionMapUpdate, StructuredWorkerTurn } from './session-map-types.mjs'

function record(value: unknown): Record<string, unknown> {
  return value !== null && typeof value === 'object' ? value as Record<string, unknown> : {}
}
function isMapState(value: unknown): value is MapState { return typeof value === 'string' && MAP_STATES.includes(value) }
function isMapStructure(value: unknown): value is MapStructure { return typeof value === 'string' && MAP_STRUCTURES.includes(value) }
function isAssistantState(value: unknown): value is AssistantMapState { return typeof value === 'string' && SAFE_ASSISTANT_STATES.has(value) }
...
export class SessionMapWorkerPool {
  workers: Map<string, SessionMapWorker>
  constructor() {
    this.workers = new Map()
  }

  enqueue<T>(key: string, execute: (worker: SessionMapWorker) => T | PromiseLike<T>): Promise<T> {
    if (!key) return Promise.reject(new Error('Map worker requires a Session Map key'))
    ...
    const run = () => {
      if (worker.disposed) throw new Error('Session Map worker has been released')
...
export function sessionMapKey(backend: unknown, threadId: unknown) {
  return backend && threadId ? `${backend}:${threadId}` : ''
}

export function sessionMapEndpoint(backend: string, threadId: string, suffix = '') {
  const base = `/studio/session-map/${encodeURIComponent(backend)}/${encodeURIComponent(threadId)}`
  return suffix ? `${base}/${suffix}` : base
}

export function normalizeSessionMap(input: unknown): SessionMap | null {
  const value = record(input)
  if (!value || !value.id || !value.backend || !value.threadId) return null
  const itemIds = new Set()
  const items: SessionMapItem[] = (Array.isArray(value.items) ? value.items as unknown[] : []).map(record)
...
export function visibleMapItems(map: SessionMap | null | undefined) {
...
export function sessionMapVisibleText(value: unknown) {
...
export function parseSessionMapUpdate(value: unknown): ParsedSessionMapUpdate {
  const text = String(value || '')
  const markerIndex = text.lastIndexOf(SESSION_MAP_UPDATE_START)
...
export function mapChildren(map: SessionMap | null | undefined, parentId: string | null = null) {
  return visibleMapItems(map)
    .filter((item) => item.parentId === parentId)
    .sort((left, right) => left.position - right.position || left.title.localeCompare(right.title))
}

export function mapItemTrail(map: SessionMap | null | undefined, itemId = map?.currentItemId) {
  if (!map || !itemId) return []
  const byId = new Map(visibleMapItems(map).map((item) => [item.id, item]))
  const trail = []
  const visited = new Set()
  let item: SessionMapItem | null | undefined = byId.get(itemId)
...
export function mapProgress(map: SessionMap | null | undefined) {
...
export function flattenSessionMap(map: SessionMap | null | undefined) {
  const rows: { item: SessionMapItem; depth: number }[] = []
  const visited = new Set()
...
export function boundedMapContext(map: SessionMap | null | undefined, { maxItems = 80 } = {}) {
  if (!map) return ''
  ...
  for (const item of visible) {
    if (item.parentId && selected.has(item.id)) selected.add(item.parentId)
  }
  const items = visible
...
export function shouldBootstrapSessionMap(map: SessionMap | null | undefined) {
  return Boolean(map && !map.lastSyncedTurnId && visibleMapItems(map).length === 0)
}

export function bootstrapMapInput(map: SessionMap | null | undefined, interactions: readonly { user?: unknown; assistant?: unknown }[] = []) {
  const recent: { user: string; assistant: string; turn?: number }[] = []
...
export function safeAssistantOperations(input: unknown): AssistantMapOperation[] {
  const value = record(input)
  const operations = Array.isArray(value?.operations) ? value.operations : []
  return operations.map(normalizeAssistantOperation).filter((operation): operation is AssistantMapOperation => operation !== null)
}

export function structuredWorkerText(turn: StructuredWorkerTurn | null | undefined) {
...
function normalizeAssistantOperation(value: unknown): AssistantMapOperation | null {
  ...
  if (operation.op === 'addItem') {
    ...
    if (!itemId || !title || !isAssistantState(operation.state)) return null
    return {
      op: 'addItem',
      itemId,
  ...
  if (operation.op === 'updateItem') {
    ...
    const normalized: AssistantMapOperation = {
      op: 'updateItem',
      itemId,
...
function nullableBoundedText(value: unknown, max: number) {
  return value == null ? null : boundedText(value, max) || null
}

function structuredWorkerError(value: unknown) {
...
export function sessionMapTurnConfiguration(map: SessionMap | null | undefined) {
  if (!map) return {}
