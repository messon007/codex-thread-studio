...
test('configured environments reuse the selected history resume and are generation-cached', () => {
  ...
  const application = readFileSync(new URL('./environment-application.mjs', import.meta.url), 'utf8')
  assert.match(application, /effects\.applied\.has\(key\)/u)
  assert.match(application, /root, threadId, backend, includeThread, excludeTurns: incremental/u)
  assert.match(application, /effects\.compatibilityError\(error\)[\s\S]*effects\.disableTail\(backend, threadId\)[\s\S]*apply\(false\)/u)
  assert.match(app, /compatibilityError: isHistoryPaginationCompatibilityError/u)
