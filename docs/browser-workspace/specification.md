...
状态：Phase 0 已实现，Phase 1–3 为设计基线  
适用产品：Codex Thread Studio  
最后更新：2026-08-11

## 1. 决策摘要

Browser Workspace 将浏览器能力作为独立工作面加入 Codex Thread Studio，但不替换现有文档审阅器。产品中长期保留三个明确分离的运行边界：

1. **文档审阅器**：安全展示 Markdown、静态 HTML、文本和图片，允许搜索、选择和批注，不执行项目 HTML。
2. **人工浏览器**：使用隔离的 Tauri WebView 打开真实网页和本地开发页面，服务于人工浏览。
3. **AI 浏览器**：使用 Playwright MCP 和专用 Chromium Profile 向 Codex/OpenCode 提供结构化浏览器工具。

第一阶段不打包或维护 Chromium fork。Tauri WebView 与 Playwright Chromium 在实现共享标签页之前被视为不同的浏览器实例；二者不能隐式共享 Cookie、DOM、历史记录或登录状态。
...
- 浏览器状态以 Workspace 为隔离边界，而不是以单个 Thread 重复创建 Profile。
...
- 浏览网页的 WebView 不获得 Studio 的 Tauri capability、文件系统能力或后端控制能力。
...
| Browser Workspace | 浏览器标签、工具栏、状态、Profile 和 Agent Bridge 的总称 |
| Viewer Tab | 由 Tauri 系统 WebView 显示、主要供人操作的标签页 |
| Agent Tab | 由 Playwright 管理的 Chromium 页面，可被 AI 工具操作 |
| Workspace | 规范化后的项目根目录；浏览器 Profile 和策略的默认隔离边界 |
...
Browser Workspace 使用与 Map、批注、收藏、文档审阅一致的右侧工作区位置，不新增永久占用聊天宽度的主导航列。入口包括：
...
顶部工具栏保持单行、短标签和图标优先：后退、前进、刷新/停止、短地址栏、外部打开、More。More 承载新建/关闭标签、复制地址、清理 Workspace 数据、DevTools 和 Viewer/Agent Handoff。所有图标必须有 Tooltip、`aria-label` 和键盘焦点状态。
...
- Browser Profile 默认按 Workspace 复用；同目录下的多个 Thread 共享登录状态和页面资源。
- Thread 保存最近关联的 Tab ID，但不拥有 Browser Profile。
- 删除 Thread 不删除 Workspace Profile。
- 清除浏览数据是显式操作，并展示影响范围。
- 应用重启后恢复标签元数据；是否恢复页面由配置和上次退出状态决定。

### 5.4 加入聊天

Browser 内容只有通过用户显式操作才进入会话：
...
### 6.1 Main WebView 与 Viewer Backend

Main WebView 只渲染 Studio UI。Viewer WebView 是原生子 WebView，不是 HTML `iframe`。普通 iframe 不是可接受的实现，因为站点可以通过 `X-Frame-Options` 或 CSP `frame-ancestors` 拒绝嵌入，而且跨域后 Studio 无法安全访问页面状态。

Tauri 在不同平台使用不同系统引擎：Windows WebView2、macOS WKWebView、Linux WebKitGTK。Viewer 的兼容性测试必须覆盖平台差异，不能把 Windows Chromium 行为视为 Linux/macOS 的保证。参考：[Tauri Process Model](https://v2.tauri.app/concept/process-model/)、[Tauri WebView API](https://v2.tauri.app/reference/javascript/api/namespacewebview/)。

Viewer 使用独立 label，例如 `browser-viewer-<opaque-id>`。它不得匹配 Main WebView 的 capability，且不允许远程页面调用 Tauri API。
...
Browser Controller 是唯一可以创建、定位、隐藏和销毁 Browser WebView 的组件。前端发送声明式命令，不直接持有文件系统、进程或任意 WebView 创建权限。
...
browser.clear_workspace_data
browser.handoff_to_agent
```
...
browser.navigation_committed
browser.navigation_finished
browser.title_changed
browser.load_failed
browser.popup_requested
browser.download_requested
browser.tab_closed
browser.agent_status_changed
```

命令和事件字段在实现前形成版本化 schema，并验证未知字段、无效 URL、过长字段和越权 Workspace。

### 6.3 Project Preview Server

项目 HTML 不通过 Studio Gateway Origin 提供。Preview Server 使用另一个随机 loopback 端口和不可猜测的 Workspace token：
...
Browser WebView 使用独立 label，并拥有空或最小 capability。不得使用通配符把 Main WebView 权限合并给 Browser WebView。

Tauri 官方说明远程来源默认不能访问 Tauri API；Linux/Android 还无法可靠区分窗口请求与内嵌 iframe 请求，因此本特性不通过 Main WebView iframe 承载远程页面。参考：[Tauri Capabilities](https://v2.tauri.app/security/capabilities/)。
...
### Phase 1：人工 Browser Workspace

范围：右侧 Browser rail、Tauri child WebView、单行工具栏、基础 Tab/导航/错误状态、HTTP(S)/localhost、HTML 双入口、Project Preview Server 和外部浏览器 fallback。

- AC-1.1：用户能在 Browser rail 打开一个 HTTPS 页面和一个 localhost 页面。
- AC-1.2：后退、前进、刷新、地址输入、复制地址和外部打开均可键盘操作。
- AC-1.3：切换 Thread 后 Tab 关联正确，切回时不无条件重建已加载页面。
- AC-1.4：现有安全 HTML 预览不变；“运行”在独立 Origin 执行 CSS、图片和 JavaScript。
- AC-1.5：Project Preview 不能读取 Workspace 根目录之外的文件。
- AC-1.6：远程页面不能访问 Studio Gateway 或 Tauri API。
- AC-1.7：拒绝加载、证书、DNS、崩溃和离线错误有明确 UI。
- AC-1.8：Linux 完成主验收；Windows 和 macOS 至少完成编译与平台行为检查。

### Phase 2：Playwright AI Browser

范围：Playwright MCP 发现/启动/状态、Workspace Profile、Agent Tab、浏览器工具、审批、Origin Policy、工件、Codex 集成和 OpenCode 兼容性验证。

- AC-2.1：同一 Workspace 重启后可选择恢复 Persistent Profile；不同 Workspace 不共享 Cookie。
- AC-2.2：AI 能打开本地应用、读取结构化页面快照、点击、输入并截图。
- AC-2.3：Console 错误和失败请求保存为完整工件，聊天只显示摘要和引用。
- AC-2.4：提交、上传、下载和非允许 Origin 的交互执行前要求确认。
- AC-2.5：拒绝审批后 Turn 能继续，并收到结构化拒绝结果。
- AC-2.6：关闭 Studio 或 Browser Broker 后不遗留失控 Chromium/MCP 进程。
- AC-2.7：并行 Thread 不会错误复用另一个 Workspace 的 Tab 或 Profile。
- AC-2.8：不支持的后端能力明确灰化并说明原因。

### Phase 3：共享浏览时间线

范围：Viewer 到 Agent Handoff、可选 Playwright MCP Bridge 扩展、页面/选择/元素/截图/日志加入聊天、localhost 服务发现、视觉比较和 Browser Timeline。

- AC-3.1：Handoff 明确说明只复制 URL，除非用户显式连接已有 Profile。
- AC-3.2：用户可以选择一个可见 Chrome 标签页授权给当前 Workspace，其他标签不可见。
- AC-3.3：选择文本或元素后加入聊天，内容包含来源 URL 且不自动发送。
- AC-3.4：截图、Console 和 Network 工件能在当前 Thread 中打开、收藏和重新引用。
- AC-3.5：AI 操作和人工操作在 Timeline 中可区分、可审计。
- AC-3.6：视觉回归保存修改前后截图及对应 Turn/commit 元数据。
...
配置加载必须接受缺失字段并应用安全默认值。所有大小、数组长度和 URL 长度都有上限。
...
- Ubuntu/WebKitGTK 是当前主开发环境；
- Windows 验证 WebView2、缩放、IME、bounds 和 WSL localhost；
- macOS 验证 WKWebView、数据存储、窗口层级、权限和快捷键；
...
- 关闭特性不删除 Profile 和工件；
- 用户可单独清除 Browser 数据；
- Phase 0、1、2 分别提交，避免把安全、UI 和 Playwright 依赖混成大提交；
...
1. 当前 Tauri 版本在 Linux、Windows、macOS 上 child WebView 的 bounds、隐藏、焦点和 IME 行为。
2. Viewer Profile/Data Directory 在三个平台上的一致性和清除方式。
