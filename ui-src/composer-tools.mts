...
export function composerTrigger(value: string, cursor = value.length): ComposerTrigger | null {
...
export function shellCommandFromComposer(value: unknown) {
  const match = String(value || '').match(/^\s*!(.*)$/su)
  return match ? match[1]!.trim() : null
}

export function matchingSlashCommands(query: unknown, commands = SLASH_COMMANDS) {
...
export function replaceComposerTrigger(value: string, trigger: ComposerTrigger | null, replacement: unknown) {
...
export function fuzzyFileLabel(file: FileLabel | null | undefined) {
  if (!file) return ''
...
export function createComposerDraftStore() {
  ...
  function write(key: unknown, value: unknown) {
  ...
  return {
    switchTo(key: unknown, currentValue = '') {
...
export function isPreviewableTextFile(file: FileLabel | null | undefined) {
...
export function isPreviewableImageFile(file: FileLabel | null | undefined) {
  const path = fuzzyFileLabel(file).replaceAll('\\', '/')
  const name = path.split('/').filter(Boolean).pop()?.toLowerCase() || ''
  const extension = name.includes('.') ? (name.split('.').pop() || '') : ''
...
export function previewableFileKind(file: FileLabel | null | undefined) {
...
export function reviewableFileKind(file: FileLabel | null | undefined) {
...
export function selectedFileReference(file: FileLabel | null | undefined) {
...
export function matchingSkills<T extends SkillReference>(query: unknown, skills: readonly T[] = []) {
  const needle = String(query || '').toLowerCase()
  return skills
...
export function selectedSkillReference(skill: SkillReference | null | undefined) {
  return skill?.name ? `$${skill.name} ` : ''
}

export function transcriptUpdateKind(method: string | null | undefined) {
