...
export function rankRouterTargets(candidates, query, prompt = '') {
    const needle = query.trim().toLocaleLowerCase();
    const terms = (prompt.toLocaleLowerCase().match(/[\p{L}\p{N}_-]+/gu) || []).flatMap(word => /[\u3400-\u9fff]/u.test(word) ? Array.from({ length: Math.max(0, word.length - 1) }, (_, i) => word.slice(i, i + 2)) : [word]).filter(word => word.length > 1);
...
export function routerMention(value, cursor) {
