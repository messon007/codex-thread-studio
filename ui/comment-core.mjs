...
export function commentRecord(value) {
...
export class CommentSourceRegistry {
    ...
    register(provider) {
        ...
        if (!id)
            throw new Error('Comment source provider requires an id');
        if (this.#providers.has(id))
            throw new Error(`Comment source provider already registered: ${id}`);
...
export function createCommentDraft(value, { idFactory = defaultIdFactory, now = () => new Date().toISOString(), registry } = {}) {
...
export function normalizeCommentDrafts(value, { idFactory = defaultIdFactory, migrateSource = () => ({ provider: 'unknown', version: 1, anchor: {} }), registry, limitPerOwner = 32, } = {}) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return {};
    return Object.fromEntries(Object.entries(value).flatMap(([ownerId, drafts]) => {
        if (!ownerId || !Array.isArray(drafts))
            return [];
...
export function commentSelectionSnapshot(value, registry) {
