...
export function shouldFollowLatestOnReturn({ orderedTurnIds = [], readingTurnId = '', recentTurnCount = 2, bottomDistance = Infinity, bottomThreshold = 48 } = {}) {
    ...
    if (readingId && ids.length)
        return nearBottom && ids.slice(-Math.max(1, recentTurnCount)).includes(readingId);
    return nearBottom;
}
export function transcriptRestorePlan({ saved, historyReady = true, historyComplete = true, orderedTurnIds = [] } = {}) {
    ...
    if (anchorTurnId) {
        ...
        if (available)
            return { type: 'anchor', anchorTurnId };
        return { type: historyComplete ? 'stale' : 'unavailable' };
    }
    if (Number.isFinite(saved.scrollTop))
        return { type: 'scrollTop', scrollTop: Number(saved.scrollTop) };
    return { type: 'stale' };
}
