import type { PresentationItem, PresentationModel } from './transcript-types.mjs'
import type { RemoteOccurrence, SessionOccurrence } from './catalog-search-types.mjs'

const MAX_SEARCHABLE_TEXT = 200_000
const SNIPPET_CONTEXT = 82

export function isUnsupportedOccurrenceSearchError(error: unknown) {
  if (Number(record(error).code) === -32601) return true
  const message = String(record(error).message || error || '')
  return /method (?:not found|unknown)|unsupported(?: method)?|does not support|not supported/iu.test(message)
}

export function localSessionOccurrences(model: PresentationModel | null, searchTerm: unknown, { includeMessages = true, includeActivity = true } = {}) {
...
export function mergeSessionOccurrences(remote: RemoteOccurrence[] = [], local: SessionOccurrence[] = []) {
  const localByItem = new Map(local.map((entry) => [`${entry.turnId}:${entry.itemId}`, entry]))
  const merged: SessionOccurrence[] = remote.map((entry, remoteIndex): SessionOccurrence => {
...
export function normalizeRemoteSessionOccurrences(remote: RemoteOccurrence[] = [], model: PresentationModel = {}): SessionOccurrence[] {
  const itemMetadata = new Map<string, Pick<SessionOccurrence, 'type' | 'turnIndex' | 'itemIndex'>>()
  ...
  return (Array.isArray(remote) ? remote : []).map((entry, remoteIndex): SessionOccurrence => {
...
export function filterSessionOccurrences<T extends RemoteOccurrence>(entries: T[] = [], type = 'all') {
  return type === 'all' ? [...entries] : entries.filter((entry) => entry.type === type)
}

export function matchedSnippet(value: unknown, searchTerm: unknown) {
...
function occurrenceType(item: PresentationItem | undefined, itemIndex: number, finalAssistantIndex: number): SessionOccurrence['type'] {
...
function findFinalAssistantIndex(items: PresentationItem[]) {
...
function searchableItemText(item: PresentationItem | null) {
