...
export class SessionDispatchRegistry {
    ...
    preparedSessions;
    preparationLoads;
    preparationGenerations;
    sessionPreparationGenerations;
    constructor() {
    ...
    register(backend, adapter) {
        ...
        if (!name)
            throw new Error('Invalid session backend');
    ...
    prepareTurn(ref, options = {}) {
        ...
        if (typeof adapter.prepareTurn !== 'function')
        ...
        const preparation = Promise.resolve(adapter.prepareTurn(session, options))
            ...
            .finally(() => {
            if (this.preparationLoads.get(session.key) === preparation)
    ...
    markPrepared(ref) {
        const session = normalizeSessionRef(ref);
        if (typeof this.adapterFor(session).prepareTurn === 'function')
            this.preparedSessions.add(session.key);
    }
    clearPreparedSession(ref) {
        const session = normalizeSessionRef(ref);
        this.sessionPreparationGenerations.set(session.key, (this.sessionPreparationGenerations.get(session.key) || 0) + 1);
    ...
    clearPrepared(backend = null) {
        ...
        for (const key of this.preparedSessions)
            if (key.startsWith(prefix))
                this.preparedSessions.delete(key);
        for (const key of this.preparationLoads.keys())
            if (key.startsWith(prefix))
                this.preparationLoads.delete(key);
    }
    ...
    adapterFor(ref) {
        ...
        if (!adapter)
            throw new Error(`Unsupported session backend: ${session.backend}`);
        return adapter;
    }
}
...
export async function startTurnWithPreparation({ registry, ref, prepare, start, recoverThreadNotFound = false, }) {
    await prepare();
    try {
        return await start();
    }
    catch (error) {
        if (!recoverThreadNotFound || !isThreadNotFoundError(error))
            throw error;
