import type { OpenCodeSession, OpenCodeMessage, OpenCodeProviderResult } from './opencode-history-types.mjs'
...
export interface FetchOptions {
  method?: string; body?: unknown; timeoutMs?: number; allowInactive?: boolean; includeHeaders?: boolean
}
export interface OpenCodeInput { type?: string; name?: string; text?: string; url?: string; path?: string; root?: string }
export interface OpenCodeRpcParams {
  threadId?: string; cwd?: string; limit?: number; model?: string; name?: string; input?: OpenCodeInput[]
  historyAnchorTurnIds?: string[]; lastTurnId?: string; developerInstructions?: string
  outputSchema?: Record<string, unknown>; command?: string; query?: string; roots?: string[]; cwds?: string[]
}
export interface ProtocolThread { id?: string; cwd?: string; model?: string }
export interface OpenCodeProtocolState {
  backend: string; ready: boolean; selectedId: string | null
  threads: ProtocolThread[]; threadsByBackend: Record<string, ProtocolThread[]>
  hiddenSessionDirectories: string[]; sessionDirectoryIgnore: string[]
}
export interface OpenCodeProtocolServices {
  gatewayFetch: (path: string, init?: RequestInit) => Promise<Response>
  selectedThread: () => ProtocolThread | null
  currentTurnOptions: () => { model?: string }
  t: (text: string, values?: Record<string, string>) => string
  studioPerformance: { start: (name: string, details: Record<string, unknown>) => (details: Record<string, unknown>) => void }
  threadCatalogKey: (backend: string, id: string | undefined) => string
  openCodeHistoryEventSequences: Map<string, number>
}
type MessageHistory = Awaited<ReturnType<typeof collectOpenCodeMessageHistory>> & { matched?: boolean; anchorTurnId?: string | null }
function serverError(value: unknown): string | undefined {
  if (!value || typeof value !== 'object') return undefined
  const object = value as Record<string, unknown>
  const error = object.error
  if (error && typeof error === 'object' && typeof (error as Record<string, unknown>).message === 'string') return (error as { message: string }).message
...
export function createOpenCodeProtocol(state: OpenCodeProtocolState, services: OpenCodeProtocolServices) {
...
async function openCodeFetch<T = unknown>(path: string, options: FetchOptions & { includeHeaders: true }): Promise<{ value: T; headers: Headers }>
async function openCodeFetch<T = unknown>(path: string, options?: FetchOptions): Promise<T>
async function openCodeFetch<T = unknown>(path: string, { method = 'GET', body, timeoutMs = 30_000, allowInactive = false, includeHeaders = false }: FetchOptions = {}): Promise<T | { value: T; headers: Headers }> {
  ...
  try {
    ...
    if (response.status === 204) return includeHeaders ? { value: null as T, headers: response.headers } : null as T
    const text = await response.text()
    let value: unknown = null
    try { value = text ? JSON.parse(text) : null } catch { value = text }
    if (!response.ok) throw new Error(serverError(value) || `${method} ${path} failed: HTTP ${response.status}`)
    return includeHeaders ? { value: value as T, headers: response.headers } : value as T
  } catch (error) {
    if (error instanceof Error && error.name === 'AbortError') throw new Error(t('{method} {path} request timed out', { method, path }))
    throw error
  } finally { clearTimeout(timer) }
}

async function fetchOpenCodeMessageHistory(threadId: string | undefined, directory: string | undefined, fetchOptions: FetchOptions, { anchorTurnIds = [] }: { anchorTurnIds?: string[] | undefined } = {}) {
  ...
  const messageSnapshots: Record<string, { afterSequence: number; ambiguousThroughSequence: number }> = {}
  const fetchPage = async ({ limit, before }: { limit: number; before?: string }) => {
    pageCount += 1
    const sequenceBefore = openCodeHistoryEventSequences.get(key) || 0
    const query = new URLSearchParams({ limit: String(limit) })
    if (before) query.set('before', before)
    const page = await openCodeFetch<OpenCodeMessage[]>(withDirectory(path, directory, query.toString()), {
      ...fetchOptions,
      includeHeaders: true,
    })
    const result = {
  ...
  try {
    ...
    let history: MessageHistory = anchors.length
...
async function fetchOpenCodeStatusSnapshot(threadId: string | undefined, directory: string | undefined, fetchOptions: FetchOptions) {
  const key = String(threadId || '')
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      value = await openCodeFetch<Record<string, unknown>>(withDirectory('/session/status', directory), fetchOptions)
    } catch {
...
async function fetchOpenCodeCatalog(limit = 100, { allowInactive = false, includeStatuses = true } = {}) {
  ...
  const sessions = await collectOpenCodeRootSessions(({ limit: pageLimit, archived, roots, cursor }) => {
    ...
    return openCodeFetch<OpenCodeSession[]>(`/experimental/session?${query}`, options)
  }, limit)
  if (!includeStatuses) return normalizeOpenCodeSessions(sessions, {})
  const directories = [...new Set((sessions || []).map((session) => session.directory).filter((cwd): cwd is string =>
    Boolean(cwd) && !isSessionDirectoryHidden(cwd, state.hiddenSessionDirectories, state.sessionDirectoryIgnore),
  ))]
  const statuses = await fetchOpenCodeDirectoryStatuses(
    directories,
    (cwd) => openCodeFetch<Record<string, unknown>>(withDirectory('/session/status', cwd), options),
  )
  return normalizeOpenCodeSessions(sessions, statuses)
}

function directoryQuery(directory: string | undefined = selectedThread()?.cwd) {
  return directory ? `directory=${encodeURIComponent(directory)}` : ''
}

function withDirectory(path: string, directory?: string, extra = '') {
  return `${path}?${[directoryQuery(directory), extra].filter(Boolean).join('&')}`
}

async function openCodeRpc(method: string, params: OpenCodeRpcParams = {}, timeoutMs = 30_000, { allowInactive = false } = {}) {
  ...
  if (method === 'thread/resume' || method === 'thread/read') {
    const session = await openCodeFetch<OpenCodeSession>(withDirectory(`/session/${encodeURIComponent(String(params.threadId))}`, directory), fetchOptions)
  ...
  if (method === 'thread/start') {
    ...
    const session = await openCodeFetch<OpenCodeSession>(withDirectory('/session', params.cwd), { method: 'POST', body: { ...(params.name ? { title: params.name } : {}), ...(model ? { model: { id: model.modelID, providerID: model.providerID } } : {}) }, timeoutMs, allowInactive })
  ...
  if (method === 'thread/fork') {
    const session = await openCodeFetch<OpenCodeSession>(withDirectory(`/session/${encodeURIComponent(String(params.threadId))}/fork`, directory), { method: 'POST', body: openCodeForkBody(params), timeoutMs })
    return { thread: normalizeOpenCodeSessions([session], {})[0] }
  }
  if (method === 'thread/delete') return openCodeFetch(withDirectory(`/session/${encodeURIComponent(String(params.threadId))}`, directory), { method: 'DELETE', timeoutMs })
  ...
  if (method === 'turn/start') {
    ...
    if (skill?.name) {
      ...
      const result = await openCodeFetch<OpenCodeMessage>(withDirectory(`/session/${encodeURIComponent(String(params.threadId))}/command`, directory), {
        method: 'POST',
  ...
  if (method === 'turn/interrupt') return openCodeFetch(withDirectory(`/session/${encodeURIComponent(String(params.threadId))}/abort`, directory), { method: 'POST', timeoutMs })
  ...
  if (method === 'fuzzyFileSearch') {
    ...
    const files = await openCodeFetch<string[]>(withDirectory('/find/file', params.roots?.[0] || directory, `query=${query}&type=file&dirs=false&limit=200`), { timeoutMs })
    return { files: (files || []).map((path) => ({ path, root: params.roots?.[0] || directory })) }
  }
  if (method === 'model/list') {
    const providers = await openCodeFetch<OpenCodeProviderResult>(withDirectory('/config/providers', directory), { timeoutMs })
    return { data: openCodeModelList(providers) }
  }
  if (method === 'skills/list') {
    const commands = await openCodeFetch<{ source?: string; name: string; description?: string }[]>(withDirectory('/command', params.cwds?.[0] || directory), { timeoutMs })
  ...
  if (method === 'mcpServerStatus/list') {
    const servers = await openCodeFetch<Record<string, unknown>>(withDirectory('/mcp', directory), { timeoutMs })
...
function openCodeFilePart(file: OpenCodeInput) {
...
function openCodeTurnResult(userId: string, input: OpenCodeInput[] | undefined) {
  return {
    turn: {
      id: userId,
      status: 'inProgress',
...
async function openCodeUserMessageBaseline(threadId: string | undefined, directory: string | undefined, fetchOptions: FetchOptions) {
  const path = `/session/${encodeURIComponent(String(threadId))}/message`
  const messages = await openCodeFetch<OpenCodeMessage[]>(withDirectory(path, directory, 'limit=20'), fetchOptions)
...
async function openCodeStartedTurn(threadId: string | undefined, directory: string | undefined, input: OpenCodeInput[] | undefined, baselineIds: Set<string>, fetchOptions: FetchOptions) {
  const path = `/session/${encodeURIComponent(String(threadId))}/message`
