...
test('Send and Clear submits the composer before clearing only the selected session comments', () => {
  ...
  assert.match(sendAndClear, /await commentSubmissions\.submit\(/u)
  assert.match(sendAndClear, /send: submitComposer/u)
  assert.match(sendAndClear, /outcome !== 'sent'/u)
  assert.match(sendAndClear, /selectedStateKey\(\) === key/u)
  assert.doesNotMatch(sendAndClear, /requestSubmit|delete state\.annotationDrafts/u)
